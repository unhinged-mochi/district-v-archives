...
export default function Boot({ onComplete }: BootProps) {
  ...
  useEffect(() => {
    ...
    const lastLine = linesContainer.children.at(-1) as HTMLElement;
    if (lastLine) {
      tl.to(lastLine, {
        textShadow:
          "0 0 20px var(--color-terminal-amber), 0 0 40px var(--color-terminal-amber)",
        duration: 0.3,
        repeat: 2,
        yoyo: true,
      });
  ...
  return (
    <button
      ...
      className="fixed inset-0 z-[9999] flex cursor-pointer flex-col items-center justify-center border-none bg-transparent p-0 text-left"
      onClick={skip}
    ...
    >
      <div
        className="font-mono text-sm text-terminal-amber leading-relaxed sm:text-base"
        ref={linesRef}
      >
        {BOOT_LINES.map((lineData, i) => (
          <div
            className={`glow-text ${
              lineData.isAccessGranted ? "mt-2 font-bold text-lg" : ""
            }`}
            key={i}
            style={{
              display: "none",
          ...
          >
            {lineData.text.split("").map((char, ci) => (
              <span className="boot-char" key={ci} style={{ opacity: 0 }}>
                {char}
              </span>
            ))}
            {lineData.status && (
              <span
                className="boot-status ml-1 font-bold"
                style={{ opacity: 0, color: "var(--color-status-safe)" }}
              >
                {" "}
                {lineData.status}
              </span>
            ...
            <span
              className="boot-cursor cursor-blink"
              style={{ display: "none" }}
            >
              _
            </span>
      ...
      <div
        className="mt-8 cursor-blink font-mono text-xs"
